...
class DQNAgent:
    ...
    def train(self, terminal_state, step):
        ...
        for index, (current_state, action, reward, new_current_state, done) in enumerate(samples):
            ...
            #print("Type of Array X",X.dtype)
            #print("Size of Array X",X.size)
            #print("+++++++++++++++++++++++++++++++++Y++++++++++++")                                    
            #print(y)            
            #print("Shape of Array y",y.shape)
            #print("Type of Array y",y.dtype)
            #print("Size of Array y",y.size)
            


        # Fit on all samples as one batch, log only on terminal state
        self.model.fit(X, y, batch_size=MINIBATCH_SIZE, verbose=0, shuffle=False, callbacks=[self.tensorboard] if terminal_state else None)
...
for episode in tqdm(range(1, 720), ascii=True, unit='episodes'):
